/* code/markdown.rs test_format_markdown — the crate's whole-document
   Markdown fixture, formatted and then linted clean. The two strings are
   extracted verbatim from the crate source (cmd/run.ts pins the version);
   regenerate with the note in src/autocorrect/readme.md if the pin moves.

   Part of the autocorrect port's test suite; TestAutocorrect() calls this. */

#include "Test.h"

// The autocorrect pair is not part of the gpui amalgam; see
// AutocorrectTests.cpp.
#include "autocorrect/autocorrect.h"

#include <stdio.h>

void TestAutocorrectMarkdownFormat(base::Arena* a) {
    TestSuite("autocorrect markdown format");
    const char* example = R"acmd(---
...
首先将数组排序（为二分查找做准备），然后对于数组中的每个 a[i]，使用 BinarySearch 的 rank() 方法对-a[i]进行二分查找。如果结果为 j 且 j＞i，我们就将计数器加 1。
    ...
    autocorrect::FormatResult r =
        autocorrect::FormatFor(a, Str(example), StrL("markdown"));
    utassert(!r.HasError());
    bool ok = base::StrEq(r.out, Str(expected));
    utassert(ok);
    if (!ok) {
        // Point at the first differing line rather than dumping 4 KB.
        Str got = r.out;
        ...
        int lineStart = i;
        while (lineStart > 0 && got.s[lineStart - 1] != '\n') {
            lineStart--;
        }
    ...
    // The corrected document lints clean.
    autocorrect::LintResult lr =
        autocorrect::LintFor(a, Str(expected), StrL("markdown"));
    utassert(!lr.HasError());
    utassert(lr.nLines == 0);
    if (lr.nLines > 0) {
